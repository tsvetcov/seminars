...
namespace GraphProcessing {
	...
	vector<int> Ford_Bellman(Graph &listGraph, vector<int> &ancestors, size_t start, size_t nights) {
		const static size_t INF = 99999999;
		size_t countVertecies = listGraph.getNumberofVertecies();
		vector<int> distance(countVertecies + 1, INF);
		vector<int> steps(countVertecies + 1, 0);
		int result = -1;
		distance[start] = 0;
		for (size_t i = 0; i < (countVertecies + 1); ++i) {
			for (size_t j = 0; j < countVertecies; ++j) {
				vector<pair<size_t, int>> neighbours = listGraph.getNeibours(j, countVertecies);
				for (size_t k = 0; k < neighbours.size();++k) {
					if (distance[neighbours[k].first] > distance[j] + neighbours[k].second && steps[j] < nights) {
						distance[neighbours[k].first] = distance[j] + neighbours[k].second;
						steps[neighbours[k].first] = steps[j] + 1;
						ancestors[neighbours[k].first] = j;
						result = neighbours[k].first;
	...
	vector<int> Avia(Graph &listGraph, vector<int> &ancestors, size_t start, size_t nights) {
		const static size_t INF = 99999999;
		size_t countVertecies = listGraph.getNumberofVertecies();
		vector<int> distance(countVertecies + 1, INF);
		vector<int> distance_temporal(countVertecies + 1, INF);
		vector<int> steps(countVertecies + 1, 0);

		int result = -1;
		distance[start] = 0;
		distance_temporal[start] = 0;
		for (size_t i = 0; i < nights; ++i) {
			for (size_t j = 0; j < countVertecies; ++j) {
				vector<pair<size_t, int>> neighbours = listGraph.getNeibours(j, countVertecies);
				for (size_t k = 0; k < neighbours.size();++k) {
					if (distance[neighbours[k].first] > distance[j] + neighbours[k].second) {
						distance_temporal[neighbours[k].first] = distance[j] + neighbours[k].second;
						steps[neighbours[k].first] = steps[j] + 1;
						ancestors[neighbours[k].first] = j;
						result = neighbours[k].first;
	...
	vector<size_t> negative_cycle(Graph &listGraph) {
		...
		vector<int> dist = Ford_Bellman(listGraph, ancestors, 0, 100);
...
int main() {
	size_t countVertecies, countEdges, nights, start, finish;
	std::cin >> countVertecies >> countEdges >> nights >> start >> finish;
	AdjListsGraph listGraph(countVertecies, countEdges);
	vector<int> ancestors(countVertecies, -1);
	vector<int> dist = Avia(listGraph, ancestors, start - 1, nights);
	for (int i = 0; i< dist.size(); ++i) {
		//cout << dist[i] << " ";
	}
	if (dist[finish - 1] < 99999998) {
		cout << dist[finish - 1];
	}
	else {
		cout << -1;
